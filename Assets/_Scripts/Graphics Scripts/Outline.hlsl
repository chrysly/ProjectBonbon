...
{
    float ns1 = snoise(float2(0,1));
    float ns2 = snoise(float2(0,1));
    float ns3 = snoise(float2(0,1));
    
    float halfScaleFloor = OutlineThickness * .05f;
    float halfScaleCeil = OutlineThickness * .05f;
    ...
    uvSamples[3] = UV + float2(-Texel.x * halfScaleFloor, Texel.y);
    ...
    float edgeDepth = abs(sqrt(pow(depthFiniteDifference0, 2) + pow(depthFiniteDifference1, 2)) * 100);
